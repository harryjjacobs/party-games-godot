...
const PlayerIcon = preload("res://memes/player/PlayerIcon.gd")
const PlayerIconScene = preload("res://memes/player/PlayerIcon.tscn")
...
func add_player(player: Player, animate = true):
	if player in _player_icon_lookup:
		return
	var player_icon = PlayerIconScene.instance()
